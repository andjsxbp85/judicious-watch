...
// Minimal domain data needed for AI chatbot
interface DomainForChat {
  domain: string;
  status: string;
...
const AIChatbot: React.FC<AIChatbotProps> = ({ domain }) => {
  ...
  const generateAIResponse = (question: string): string => {
    ...
    if (q.includes("alasan") || q.includes("kenapa") || q.includes("mengapa")) {
      return domain.reasoning || "Tidak ada informasi reasoning tersedia.";
    }
    ...
    if (
      ...
      return `Confidence score untuk domain ini adalah **${
    ...
    if (q.includes("status") || q.includes("klasifikasi")) {
      return `Domain ini diklasifikasikan sebagai **${
        domain.status === "judol" ? "Judi Online" : "Non Judi Online"
      }** dengan confidence score ${domain.confidenceScore}%.`;
    }

    return `Berdasarkan analisis saya terhadap **${domain.domain}**:\n\n${
      domain.reasoning || "Tidak ada informasi reasoning tersedia."
  ...
  return (
    <div id="ai-chatbot-container" className="flex flex-col h-full">
      ...
      >
        ...
        <div>
          <h3 id="ai-chatbot-title" className="font-semibold text-foreground">
            AI Assistant
          </h3>
          <p id="ai-chatbot-subtitle" className="text-xs text-muted-foreground">
            Analisis Domain
          </p>
        </div>
      </div>
