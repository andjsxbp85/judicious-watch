...
export const DomainsProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  ...
  const updateDomain = useCallback(
    (domainId: string, updates: Partial<Domain>) => {
      setDomains((prev) =>
        prev.map((d) => (d.id === domainId ? { ...d, ...updates } : d))
      );
    },
    []
  );
  ...
  const getStatistics = useCallback(() => {
    ...
    const totalUrls = domains.reduce(
      (sum, d) => sum + (d.urlGroup?.length || 1),
      0
    );
    const judolCount = domains.filter((d) => d.status === "judol").length;
    const nonJudolCount = domains.filter(
  ...
  const getTrendData = useCallback((): TrendData[] => {
    ...
    domains.forEach((d) => {
      if (d.verifiedAt) {
        ...
        const dateStr = verifiedDate.toLocaleDateString("id-ID", {
          day: "2-digit",
          month: "short",
        });
  ...
  const getVerifikatorStats = useCallback((): VerifikatorStats[] => {
    ...
    domains.forEach((d) => {
      if (d.verifiedBy && d.status !== "manual-check") {
  ...
  const getRecentVerifications = useCallback(
    (limit = 5): Domain[] => {
      return domains
